...
from dataset import resize, ImageStandardizer, PostersDataset
from utils import config, denormalize_image

training_set = PostersDataset('train', config('autoencoder.num_classes'))
...
while True:
    ...
    for i, (xi, yi) in enumerate(zip(X_, y)):
        axes[1,i].imshow(xi, interpolation='bicubic')
