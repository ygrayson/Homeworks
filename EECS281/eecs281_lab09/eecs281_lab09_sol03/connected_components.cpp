...
//Algorithm 3 - same as algorithm 2 but using adjacency matrix
...
public:
	...
	Graph(unsigned int V, unsigned int E) : V(V), E(E), explored(V, false), 
		adjmatrix(V, vector<bool>(V, false)), num_connected(0) {}
	...
	for (unsigned int i = 0; i < explored.size(); ++i) {
		if (explored[i] == false) {
			...
			while (!connected.empty()) {
				...
				for (unsigned int j = 0; j < adjmatrix[current].size(); ++j) {
					if(adjmatrix[current][j] && !explored[j])
						connected.push(j);
				}
			}
		}
